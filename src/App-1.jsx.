...
function AuthScreen({ onAuthed }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onAuthed({ token: data.token, name: data.user.name, phone: data.user.phone });
    } catch (e) {
...
function buildAiSystemPrompt(products) {
  const list = products.map((p) => `- "${p.title}" (${CATEGORY_LABEL[p.category] || p.category}) — ${p.priceXof} FCFA`).join("\n");
  return `Tu es VORTEX AI, l'assistant intégré de la plateforme VORTEX pour entrepreneurs et traders d'Afrique francophone.
Tu réponds toujours en français, de façon chaleureuse, concise et concrète.
Catalogue actuel :
${list || "(catalogue vide pour le moment)"}
Si tu recommandes un contenu, cite son titre exact. Reste bref (3-5 phrases sauf si on te demande plus de détail).`;
}

function AIAssistant({ onClose, products }) {
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Salut ! Je suis VORTEX AI 👋 Je peux te recommander une formation ou un ebook, ou répondre à tes questions. Que puis-je faire pour toi ?" },
  ]);
  ...
  const send = async () => {
    ...
    setInput("");
    setLoading(true);
    // NB: VORTEX AI appelait directement l'API Claude depuis l'aperçu Claude
    // (où c'est proxifié automatiquement, sans clé exposée). Sur le site
    // déployé, un appel direct depuis le navigateur exigerait d'exposer une
    // clé API publiquement — à éviter. Il faut un petit endpoint backend
        // (ex: POST /ai/chat sur vortex-backenda.onrender.com) qui garde la clé
    // côté serveur. Tant que cet endpoint n'existe pas, on l'indique clairement.
    await new Promise((r) => setTimeout(r, 500));
    setMessages((m) => [...m, {
      role: "assistant",
      content: "VORTEX AI n'est pas encore branché sur ce site déployé — cette fonctionnalité a besoin d'un petit endpoint backend qui garde la clé API côté serveur (jamais exposée dans le navigateur). On peut l'ajouter à ton API Render quand tu veux.",
    }]);
    setLoading(false);
  };
...
function MarketScreen({ onBack, balance, onBuy, purchasedIds, products, loadingProducts, onOpenContent }) {
  ...
  return (
    <div className="pb-4">
      ...
      {selected && (
        <ProductSheet product={selected} owned={purchasedIds.includes(selected.id)} balance={balance}
...
function ProductSheet({ product, owned, balance, onClose, onBuy, onOpen }) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-end">
      ...
      <div className="relative w-full bg-[#182238] rounded-t-3xl border-t border-white/10 px-5 pt-4 pb-8">
        <div className="w-10 h-1 bg-white/15 rounded-full mx-auto mb-4" />
        ...
        {owned ? (
          consumable ? (
            ...
            <button className="w-full bg-[#2E9E83] text-[#0F1626] font-semibold rounded-xl py-3 text-center text-sm flex items-center justify-center gap-2"><Download size={16} /> Télécharger</button>
          )
        ) : (
          <>
...
function ProfileScreen({ name, phone, balance, purchases, progress, onBack, onOpenContent, onLogout }) {
  return (
    <div className="pb-4">
        ...
        <div className="flex-1 overflow-y-auto relative">
          ...
          {auth && (
            <>
              ...
              {screen === "market" && (
                <MarketScreen onBack={() => goTo("home")} balance={balance} onBuy={handleBuy} purchasedIds={purchasedIds}
                  products={products} loadingProducts={loadingProducts} onOpenContent={setViewing} />
              )}
              {screen === "community" && (
              ...
              {screen === "profile" && (
                <ProfileScreen name={auth.name} phone={auth.phone} balance={balance} purchases={purchases} progress={local.progress}
                  onBack={() => goTo("home")} onOpenContent={setViewing} onLogout={handleLogout} />
              )}
