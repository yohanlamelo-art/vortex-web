...
function AuthScreen({ onAuthed, lang, setLang }) {
  ...
  return (
    <div className="h-full flex flex-col justify-center px-6 relative">
      <button onClick={() => setLang(lang === "fr" ? "en" : "fr")} className="absolute top-4 right-4 text-[#F6F2E9]/50 text-xs border border-white/10 rounded-full px-3 py-1">
        {lang === "fr" ? "EN" : "FR"}
      </button>
      ...
      <div className="flex bg-white/[0.04] rounded-xl p-1 mb-5">
        <button onClick={() => setMode("login")} className={`flex-1 text-sm font-medium rounded-lg py-2 ${mode === "login" ? "bg-[#D6A337] text-[#0F1626]" : "text-[#F6F2E9]/50"}`}>{t("auth_login")}</button>
        <button onClick={() => setMode("register")} className={`flex-1 text-sm font-medium rounded-lg py-2 ${mode === "register" ? "bg-[#D6A337] text-[#0F1626]" : "text-[#F6F2E9]/50"}`}>{t("auth_register")}</button>
      </div>
      ...
      <div className="space-y-3">
        ...
        {mode === "register" && (
          <select value={country} onChange={(e) => setCountry(e.target.value)}
            ...
            <option value="NG" className="bg-[#182238]">Nigeria</option>
            <option value="GH" className="bg-[#182238]">Ghana</option>
          </select>
        )}
  ...
  return (
    <div className="pb-4">
      ...
      <div className="px-5 mt-3">
        <p className="text-[#F6F2E9]/50 text-sm">{t("home_greeting")} {name} 👋</p>
        <h2 className="text-[#F6F2E9] text-2xl font-semibold mt-0.5">{t("home_tagline")}</h2>
      </div>
      <button onClick={() => goTo("wallet")} className="mx-5 mt-5 w-[calc(100%-2.5rem)] relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#182238] to-[#101828] border border-white/5 p-5 text-left group">
        ...
        <p className="text-[#F6F2E9]/50 text-xs uppercase tracking-wider">{t("home_balance")}</p>
        <p className="text-[#F6F2E9] text-3xl font-semibold mt-1.5 tabular-nums">{fmtCur(balance, currency)}</p>
        <div className="mt-4 flex items-center gap-1 text-[#D6A337] text-xs font-medium">{t("home_manage")} <ChevronRight size={14} /></div>
      </button>
      <div className="px-5 mt-5 grid grid-cols-2 gap-3">
        <button onClick={() => goTo("wallet", "deposit")} className="rounded-2xl bg-[#182238] border border-white/5 p-4 text-left active:scale-[0.98] transition-transform">
          <ArrowDownLeft size={18} className="text-[#2E9E83]" /><p className="text-[#F6F2E9] text-sm font-medium mt-2">{t("home_deposit")}</p><p className="text-[#F6F2E9]/40 text-xs">{t("home_via_mm")}</p>
        </button>
        <button onClick={() => goTo("market")} className="rounded-2xl bg-[#182238] border border-white/5 p-4 text-left active:scale-[0.98] transition-transform">
          <ShoppingBag size={18} className="text-[#D6A337]" /><p className="text-[#F6F2E9] text-sm font-medium mt-2">{t("home_explore")}</p><p className="text-[#F6F2E9]/40 text-xs">{t("home_market")}</p>
        </button>
      </div>
      <div className="px-5 mt-6 flex items-center justify-between">
        <h3 className="text-[#F6F2E9] text-sm font-semibold">{t("home_recent")}</h3>
        <button onClick={() => goTo("wallet")} className="text-[#D6A337] text-xs">{t("home_seeall")}</button>
      </div>
  ...
  return (
    <div className="pb-4">
      ...
      <div className="mx-5 relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#182238] to-[#101828] border border-white/5 p-6">
        ...
        <p className="text-[#F6F2E9]/50 text-xs uppercase tracking-wider">{t("wallet_available")}</p>
        <p className="text-[#F6F2E9] text-4xl font-semibold mt-2 tabular-nums">{fmtCur(balance, currency)}</p>
        <div className="mt-5 flex gap-3">
          <button onClick={openDeposit} className="flex-1 bg-[#D6A337] text-[#0F1626] text-sm font-semibold rounded-xl py-2.5 flex items-center justify-center gap-1.5 active:scale-[0.97] transition-transform"><Plus size={16} /> {t("wallet_deposit")}</button>
          <button onClick={openWithdraw} className="flex-1 bg-white/10 text-[#F6F2E9] text-sm font-semibold rounded-xl py-2.5 flex items-center justify-center gap-1.5 active:scale-[0.97] transition-transform"><ArrowUpRight size={16} /> {t("wallet_withdraw")}</button>
        </div>
      </div>
      <div className="px-5 mt-6">
        <h3 className="text-[#F6F2E9] text-sm font-semibold mb-2">{t("wallet_history")}</h3>
        ...
        {!loadingProducts && list.map((p) => {
          ...
          return (
            <button key={p.id} onClick={() => setSelected(p)} className="w-full flex items-center gap-3 bg-[#182238] border border-white/5 rounded-2xl p-3.5 text-left active:scale-[0.99] transition-transform">
              ...
              {owned ? <span className="text-[#2E9E83] text-xs font-semibold shrink-0">Acquis</span> : <span className="text-[#F6F2E9] text-sm font-semibold shrink-0 tabular-nums">{fmtCur(p.priceXof, currency)}</span>}
            </button>
          );
        })}
    ...
    <div className="absolute inset-0 z-40 flex items-end">
      ...
      <div className="relative w-full bg-[#182238] rounded-t-3xl border-t border-white/10 px-5 pt-4 pb-8 max-h-[90%] overflow-y-auto">
        ...
        {owned ? (
          ...
          <>
            <div className="flex items-center justify-between mb-3"><span className="text-[#F6F2E9]/50 text-sm">Prix</span><span className="text-[#F6F2E9] text-xl font-semibold tabular-nums">{fmtCur(product.priceXof, currency)}</span></div>
              ...
              {data.products.map((p) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-3 bg-[#182238] border border-white/5 rounded-2xl p-3.5">
                    ...
                    <span className="text-[#F6F2E9] text-sm font-semibold shrink-0 tabular-nums">{fmtCur(p.priceXof, currency)}</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div className="pb-4">
      ...
      <div className="px-5 grid grid-cols-2 gap-3 mb-6">
        <div className="bg-[#182238] rounded-2xl p-4 border border-white/5"><p className="text-[#F6F2E9]/40 text-xs">Solde Wallet</p><p className="text-[#F6F2E9] text-lg font-semibold mt-1 tabular-nums">{fmtCur(balance, currency)}</p></div>
      ...
      <div className="px-5 mb-6">
        <h3 className="text-[#F6F2E9] text-sm font-semibold mb-2">{t("profile_settings")}</h3>
        <div className="bg-[#182238] rounded-2xl p-4 border border-white/5 space-y-3">
          <div>
            <p className="text-[#F6F2E9]/40 text-xs mb-1.5">Langue / Language</p>
            <div className="flex gap-2">
              <button onClick={() => setLang("fr")} className={`flex-1 text-xs font-medium rounded-lg py-2 ${lang === "fr" ? "bg-[#D6A337] text-[#0F1626]" : "bg-white/5 text-[#F6F2E9]/60"}`}>Français</button>
              <button onClick={() => setLang("en")} className={`flex-1 text-xs font-medium rounded-lg py-2 ${lang === "en" ? "bg-[#D6A337] text-[#0F1626]" : "bg-white/5 text-[#F6F2E9]/60"}`}>English</button>
            </div>
          </div>
          <div>
            <p className="text-[#F6F2E9]/40 text-xs mb-1.5">Devise affichée</p>
            <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-[#F6F2E9] outline-none">
              {Object.entries(CURRENCIES).map(([code, c]) => (
                <option key={code} value={code} className="bg-[#182238]">{c.label}</option>
              ))}
            </select>
            <p className="text-[#F6F2E9]/25 text-[10px] mt-1.5">Les montants réels restent en Franc CFA (XOF) — ceci n'est qu'un affichage indicatif.</p>
          </div>
        </div>
      </div>
      <div className="px-5 mb-6">
        <h3 className="text-[#F6F2E9] text-sm font-semibold mb-2">{t("profile_library")}</h3>
        <div className="space-y-2">
