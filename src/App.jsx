...
function AIAssistant({ onClose, token }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "assistant", content: e.message || "Une erreur est survenue. Réessaie dans un instant." }]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto relative">
          ...
          {auth && (
            <>
              ...
              {aiOpen && <AIAssistant onClose={() => setAiOpen(false)} token={auth.token} />}
