...
const CREATORS = [
  { id: "c1", name: "Aïcha Diallo", bio: "Coach en Analyse Technique", color: "#6C2BD9" },
  ...
  { id: "c4", name: "YOS Vortex", bio: "Équipe pédagogique VORTEX", color: "#6C2BD9" },
];
      ...
      <div className="px-5 mb-4">
        ...
        <div className="flex gap-2 overflow-x-auto -mx-1 px-1 pb-1">
          {CREATORS.map((c) => {
            ...
            return (
              <div key={c.id} className="shrink-0 w-32 bg-[#1C1626] border border-white/5 rounded-2xl p-3">
                ...
                <p className="text-[#F2EEFB]/40 text-[10px] truncate mb-2">{c.bio}</p>
                <button onClick={() => onToggleFollow(c.id)} className={`w-full flex items-center justify-center gap-1 text-[10px] font-semibold rounded-lg py-1.5 ${isFollowed ? "bg-white/10 text-[#F2EEFB]/60" : "bg-[#6C2BD9] text-white"}`}>
      ...
      <div className="px-5">
        ...
        <div className="space-y-3">
          {POSTS.map((post) => {
            ...
            return (
              <div key={post.id} className="bg-[#1C1626] border border-white/5 rounded-2xl p-4">
                <div className="flex items-center gap-2.5 mb-2.5">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-[#F2EEFB] text-sm font-medium truncate">{creator.name}</p><p className="text-[#F2EEFB]/35 text-[11px]">{post.date}</p></div>
                  <span className="text-[10px] font-medium text-[#6C2BD9] bg-[#6C2BD9]/10 rounded-full px-2 py-0.5 shrink-0">{post.tag}</span>
                </div>
                <p className="text-[#F2EEFB]/80 text-sm leading-relaxed mb-3">{post.content}</p>
                <div className="flex items-center gap-4">
                  <button onClick={() => onToggleLike(post.id)} className="flex items-center gap-1.5"><Heart size={16} className={isLiked ? "text-[#E0654A] fill-[#E0654A]" : "text-[#F2EEFB]/40"} /><span className={`text-xs ${isLiked ? "text-[#E0654A]" : "text-[#F2EEFB]/40"}`}>{post.likes + (isLiked ? 1 : 0)}</span></button>
                  <div className="flex items-center gap-1.5"><MessageCircle size={16} className="text-[#F2EEFB]/40" /><span className="text-xs text-[#F2EEFB]/40">{post.comments}</span></div>
                </div>
              </div>
            );
          })}
        ...
        {data && (
          <>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-16 h-16 rounded-full bg-[#6C2BD9]/20 border border-[#6C2BD9]/40 flex items-center justify-center text-[#6C2BD9] text-xl font-semibold shrink-0 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              <div className="bg-[#1C1626] rounded-xl p-3 text-center border border-white/5">
  ...
  return (
    <div className="pb-8">
      ...
      <div className="px-5 space-y-3">
        ...
        <button onClick={submit} disabled={!valid || saving} className="w-full bg-[#6C2BD9] disabled:opacity-30 text-white font-semibold rounded-xl py-3 flex items-center justify-center gap-2 mt-2">
  ...
  return (
    <div className="pb-4">
      ...
      <div className="px-5 flex items-center gap-3 mb-6">
        <div className="w-16 h-16 rounded-full bg-[#6C2BD9]/20 border border-[#6C2BD9]/40 flex items-center justify-center text-[#6C2BD9] text-xl font-semibold">{name.slice(0, 1).toUpperCase()}</div>
        <div><p className="text-[#F2EEFB] font-semibold">{name}</p><p className="text-[#F2EEFB]/40 text-xs">{phone}</p></div>
      </div>
      <div className="px-5 grid grid-cols-2 gap-3 mb-6">
        <div className="bg-[#1C1626] rounded-2xl p-4 border border-white/5"><p className="text-[#F2EEFB]/40 text-xs">Solde Wallet</p><p className="text-[#F2EEFB] text-lg font-semibold mt-1 tabular-nums">{fmtCur(balance, currency)}</p></div>
        <div className="bg-[#1C1626] rounded-2xl p-4 border border-white/5"><p className="text-[#F2EEFB]/40 text-xs">Contenus acquis</p><p className="text-[#F2EEFB] text-lg font-semibold mt-1">{purchases.length}</p></div>
      </div>
...
export default function VortexApp() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#080611] flex items-center justify-center p-4" style={{ fontFamily: "'Poppins', system-ui, sans-serif" }}>
      <div className="relative w-full max-w-[380px] h-[780px] bg-[#0B0714] rounded-[2.5rem] border-8 border-[#1A1424] shadow-2xl overflow-hidden flex flex-col">
        <div className="h-6 shrink-0 flex items-center justify-center relative"><div className="w-24 h-4 bg-[#1A1424] rounded-b-2xl absolute top-0" /></div>
          ...
          {auth && (
            <>
              ...
              {!modal && !viewing && !viewingShop && !adminOpen && !aiOpen && (
                <button onClick={() => setAiOpen(true)} className="absolute right-4 bottom-4 rounded-full bg-[#6C2BD9] premium-glow flex items-center justify-center active:scale-95 transition-transform" style={{ width: 52, height: 52 }} aria-label="Ouvrir VORTEX AI">
